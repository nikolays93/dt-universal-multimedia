...
add_filter( 'queries_forms', 'MB\_queries_forms', 10, 1 );
function _queries_forms( $form ){
	$public_args = array('public' => 1, 'show_ui' => 1);
	$qry = WPForm::active('_'.DTM_PREFIX.'query', false, true, true);
...
add_filter( 'queries_forms', 'MB\_default_queries_forms', 15, 1 );
...
add_filter( 'render_terms', 'MB\_terms_fieldgroup', 10 );
...
add_action('before_admin_wrap_attachments', 'MB\get_admin_pre_wrap_attachments', 10, 1 );
			...
			<div id="dt-media-query" style="padding: 5px 15px;display: <?php echo $display; ?>;">
				<div class="col-2" style="width: 50%; float: left;">
					...
					WPForm::render(
						...
						WPForm::active('_'.DTM_PREFIX.'query', false, true, true),
						true,
						array(
				...
				<div class="col-2" style="width: 50%; float: left;">
					...
					if( $render_terms = apply_filters( 'render_terms', array() ) )
						WPForm::render( $render_terms,
							WPForm::active('_'.DTM_PREFIX.'query', 'term', true, true),
							false,
...
add_action( 'mb_media_buttons', 'MB\add_select_query', 20, 1 );
